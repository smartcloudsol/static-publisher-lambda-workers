...
- `network.createDynamoDbGatewayEndpoint` defaults to `true` and keeps worker
  progress writes on the AWS network without requiring a NAT gateway. It uses
  the same explicit route table IDs as the S3 endpoint. Disable it explicitly
  when an equivalent shared endpoint is managed outside this stack; gateway
  endpoints have no additional hourly or data processing charge.
